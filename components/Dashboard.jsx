...
const StatCard = ({ icon, value, label, color, bg }) => (
  <div style={{
    ...
    boxShadow: "0 2px 16px rgba(10,33,86,0.07)",
    ...
    <div>
      ...
      <div style={{ fontSize: 12, color: "var(--gray)", fontWeight: 600, marginTop: 3 }}>{label}</div>
    </div>
  </div>
);
...
const Dashboard = () => {
  ...
  const bgStyle = {
    minHeight: "100vh",
    background: "var(--bg)",
    backgroundImage: `radial-gradient(circle at 20% 10%, rgba(21,101,192,0.05) 0%, transparent 40%)`,
  };

  const sectionTitle = (text) => (
    <div style={{
      fontSize: 14, fontWeight: 800, color: "#0A2156",
      textTransform: "uppercase", letterSpacing: 1,
      marginBottom: 14, display: "flex", alignItems: "center", gap: 8,
    }}>
      <div style={{ height: 3, width: 20, background: "#1565C0", borderRadius: 999 }} />
      {text}
    </div>
  );
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "36px 24px 80px" }}>
        ...
        <div style={{ marginBottom: 32 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div>
              <h1 style={{ fontSize: 28, fontWeight: 900, color: "#0A2156", margin: "0 0 4px", letterSpacing: -0.5 }}>
                Dashboard Aspirasi
              </h1>
              <p style={{ color: "var(--gray)", margin: 0, fontSize: 14 }}>
                Monitoring aspirasi & sentimen BPPTIK Komdigi
              </p>
            </div>
            <div style={{
              background: "#fff", borderRadius: 10, padding: "8px 16px",
              border: "1px solid var(--border)", fontSize: 13, color: "var(--gray)", fontWeight: 600,
            }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, marginBottom: 28 }}>
          <StatCard icon="🎫" value={stats.total}   label="Total Tiket"  color="#1565C0" bg="#EFF6FF" />
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 14, marginBottom: 28 }}>
          {[
            ...
            { icon:"💬", value:stats.komentar, label:"Komentar", desc:"Pendapat & apresiasi",   color:"#1D4ED8", bg:"#EFF6FF", border:"#BFDBFE" },
          ].map((item) => (
            <div key={item.label} style={{
              background: "#fff", borderRadius: 16, padding: "20px 24px",
              border: `1px solid ${item.border}`,
              boxShadow: "0 2px 16px rgba(10,33,86,0.06)",
              display: "flex", alignItems: "center", gap: 16,
            }}>
              <div style={{
              ...
              <div key={d.id} style={{
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <span style={{ fontWeight: 800, fontSize: 15, color: "#0A2156" }}>{d.label}</span>
                  </div>
                  <span style={{
                ...
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  {[
                    ...
                    { label:"Komentar", bg:"#EFF6FF", color:"#1D4ED8" },
                  ].map((c) => (
        ...
        <div style={{
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
            <span style={{ fontSize: 12, fontWeight: 700, color: "var(--gray)", minWidth: 60 }}>Divisi:</span>
            {[{ id:"all", label:"Semua", icon:"🗂️" }, ...DIVISIONS].map((d) => (
              <button key={d.id} onClick={() => setActiveDiv(d.id)} style={{
                padding: "6px 14px", borderRadius: 8,
                border: activeDiv === d.id ? `2px solid ${d.color || "#1565C0"}` : "2px solid var(--border)",
                background: activeDiv === d.id ? (d.bg || "#EFF6FF") : "#fff",
                color: activeDiv === d.id ? (d.color || "#1565C0") : "#374151",
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
            <span style={{ fontSize: 12, fontWeight: 700, color: "var(--gray)", minWidth: 60 }}>Kategori:</span>
            {[
              { id:"all",      label:`Semua (${tickets.length})`,       color:"#1565C0", bg:"#EFF6FF" },
              { id:"Kritik",   label:`🔴 Kritik (${stats.kritik})`,     color:"#991B1B", bg:"#FEF2F2" },
              { id:"Saran",    label:`💡 Saran (${stats.saran})`,       color:"#92400E", bg:"#FFFBEB" },
              { id:"Komentar", label:`💬 Komentar (${stats.komentar})`, color:"#1D4ED8", bg:"#EFF6FF" },
            ].map((item) => (
              <button key={item.id} onClick={() => setActiveCat(item.id)} style={{
                padding: "6px 14px", borderRadius: 8,
                border: activeCat === item.id ? `2px solid ${item.color}` : "2px solid var(--border)",
        ...
        {filtered.length === 0 ? (
          <div style={{
            ...
            <p style={{ color: "var(--gray)", fontWeight: 600 }}>Tidak ada tiket ditemukan</p>
          </div>
        ) : (
            ...
            {filtered.map((t) => {
              ...
              return (
                <div key={t.id} style={{
                  ...
                  boxShadow: "0 1px 10px rgba(10,33,86,0.06)",
                  border: "1px solid var(--border)",
                  borderLeft: `4px solid ${div?.color || "#1565C0"}`,
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
                    ...
                    <span style={{ fontSize: 11, color: "var(--gray)", fontFamily: "monospace", fontWeight: 700, letterSpacing: 0.5 }}>
                      {t.id}
                    </span>
                  </div>
                  <p style={{ color: "#374151", lineHeight: 1.65, margin: 0, fontSize: 14 }}>{t.message}</p>
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "var(--gray)", paddingTop: 4, borderTop: "1px solid #F1F5F9" }}>
                    <span>👤 {t.name}</span>
