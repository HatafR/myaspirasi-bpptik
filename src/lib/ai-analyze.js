...
export async function analyzeTextAI(text) {
  if (!process.env.GEMINI_API_KEY) return ruleBased(text);

  try {
    const prompt = `Analisa teks: "${text}". 
    Kembalikan JSON dengan format: {"sentimen": "Positif"|"Netral"|"Negatif", "kategori": "Kritik"|"Saran"|"Komentar"}`;
    ...
    // Mengambil hasil teks JSON dari response SDK baru
    ...
    return ruleBased(text); // Fallback jika gagal
  }
}
